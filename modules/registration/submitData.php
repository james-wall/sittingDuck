    ...
    if(isset($_POST['submit'])){
      ...
      $dup = mysqli_query($con, "SELECT username FROM users WHERE username='".$username."'");
        if(mysqli_num_rows($dup) >0){
            ...
            echo "<script type='text/javascript'>document.location.href = '/sittingDuckLogin/modules/registration/reg.php';</script>";
        }
        else{
          $sql = mysqli_query($con, "INSERT INTO `sittingduckclients`.`users` (`id`, `level`, `username`, `password`, `firstName`, `lastName`) VALUES (NULL, '$accountType', '$username', MD5('$userPassword'), '$firstName', '$lastName');"); 
        }
        header('Location: /sittingDuckLogin/connect.php');
